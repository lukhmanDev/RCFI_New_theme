...
@section('content')
            ...
            <div style="padding: 1.5rem;">
                ...
                <div style="margin-top: 1.5rem; background: var(--panel-bg, rgba(255, 255, 255, 0.03)); border: 1px solid var(--panel-border, rgba(255, 255, 255, 0.1)); border-radius: 8px; padding: 1rem 1.25rem; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 1rem;">
                    <div style="display: flex; align-items: center; gap: 0.75rem;">
                        <div style="background: var(--accent-green, #10b981); color: #ffffff; width: 40px; height: 40px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 1.3rem; flex-shrink: 0;">
            ...
            grid-template-columns: 1fr;
                ...
                <div style="background: rgba(255,255,255,0.02); border: 1px solid var(--panel-border); padding: 1.5rem; border-radius: 8px;">
                    ...
                    @php
                        ...
                        $mList = $pFilesData['materials'] ?? ($project->materials ?? []);
                        $s4MaterialsTotal = 0;
                        if (is_array($mList)) {
                            foreach ($mList as $mItem) {
                                $s4MaterialsTotal += floatval($mItem['amount'] ?? 0);
                            }
                        }

                        $cList = $pFilesData['community_contributions'] ?? ($project->community_contributions ?? []);
                        $s4CommTotal = 0;
                        if (is_array($cList)) {
                            foreach ($cList as $cItem) {
                                $s4CommTotal += floatval($cItem['amount'] ?? 0);
                            }
                        }
                        ...
                        $deductions = floatval($compDetails['deductions'] ?? 0);
                        $totalProjectCost = $displayGrants + $displayComm + $leverage + $anyOther - $deductions;
                    @endphp
                            ...
                            @if($isCoo || $isSuperAdmin)
                                <form action="{{ route('projects.approve', $project->id) }}" method="POST" style="margin: 0; display: flex; flex-direction: column; gap: 1rem; align-items: flex-start;">
                                    ...
                                    <div style="width: 100%; max-width: 500px;">
                                        ...
                                        <textarea name="remarks" id="remarks" rows="3" placeholder="Enter final approval remarksâ€¦" style="width: 100%; background-color: var(--bg-color); border: 1px solid var(--panel-border); color: #ffffff; padding: 0.75rem; border-radius: 6px; font-size: 0.85rem; outline: none; resize: vertical;" required></textarea>
                                    </div>
                                    <button type="submit" class="btn-custom" style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); border: none; color: #ffffff; cursor: pointer; font-weight: 700; padding: 0.6rem 1.8rem; border-radius: 6px;">
                                        âœ“ Finalize Project Approval & Complete
                                    </button>
                                </form>
                            @else
                                <div style="background-color: rgba(245, 158, 11, 0.15); border: 1px solid rgba(245, 158, 11, 0.3); color: #f59e0b; padding: 0.85rem 1.25rem; border-radius: 6px; font-size: 0.9rem; font-weight: 600; display: inline-block;">
    ...
    <script>
        ...
        function updateRealtimeApplicationDetails(selectedId) {
            ...
            const occupation = formatVal(getVal(['occupation']));
            ...
            const houseName = formatVal(getVal(['house_name']));
            const houseType = formatVal(getVal(['house_type']));
            const accommodation = formatVal(getVal(['accommodation_details', 'accommodation']));
            const ownPlace = formatVal(getVal(['own_place', 'own_place_details']));
            const landType = formatVal(getVal(['land_type']));

            const numChildren = formatVal(getVal(['num_children', 'children_total']));
            const healthStatus = formatVal(getVal(['health_status']));
            const monthlyIncome = getVal(['monthly_income']) ? '&#x20B9;' + Number(getVal(['monthly_income'])).toLocaleString() : 'N/A';

            const desiredModel = formatVal(getVal(['desired_model']));
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem;">
                    ...
                    <div>
                        ...
                        <table style="width: 100%; border-collapse: collapse; font-size: 0.85rem; color: var(--text-main);">
                            ...
                            <tr style="border-bottom: 1px solid rgba(255,255,255,0.02);"><td style="padding: 0.5rem 0; font-weight: 600; color: var(--text-muted);">Occupation:</td><td>${occupation}</td></tr>
                            ...
                            <tr style="border-bottom: 1px solid rgba(255,255,255,0.02);"><td style="padding: 0.5rem 0; font-weight: 600; color: var(--text-muted);">Post Office:</td><td>${post}</td></tr>
                        ...
                        <h4 style="color: var(--accent-cyan); border-bottom: 1px solid var(--panel-border); padding-bottom: 0.5rem; margin-top: 1.5rem; margin-bottom: 0.75rem; font-size: 0.9rem; font-weight: 700; text-transform: uppercase;">2. House & Land Information</h4>
                        <table style="width: 100%; border-collapse: collapse; font-size: 0.85rem; color: var(--text-main);">
                            <tr style="border-bottom: 1px solid rgba(255,255,255,0.02);"><td style="padding: 0.5rem 0; font-weight: 600; width: 140px; color: var(--text-muted);">House Name:</td><td>${houseName}</td></tr>
                            <tr style="border-bottom: 1px solid rgba(255,255,255,0.02);"><td style="padding: 0.5rem 0; font-weight: 600; color: var(--text-muted);">House Type:</td><td>${houseType}</td></tr>
                            <tr style="border-bottom: 1px solid rgba(255,255,255,0.02);"><td style="padding: 0.5rem 0; font-weight: 600; color: var(--text-muted);">Accommodation:</td><td>${accommodation}</td></tr>
                            <tr style="border-bottom: 1px solid rgba(255,255,255,0.02);"><td style="padding: 0.5rem 0; font-weight: 600; color: var(--text-muted);">Own Place:</td><td>${ownPlace}</td></tr>
                            <tr style="border-bottom: 1px solid rgba(255,255,255,0.02);"><td style="padding: 0.5rem 0; font-weight: 600; color: var(--text-muted);">Land Type:</td><td>${landType}</td></tr>
                        </table>
                    </div>

                    <!-- Col 2 -->
                    <div>
                        <h4 style="color: var(--accent-cyan); border-bottom: 1px solid var(--panel-border); padding-bottom: 0.5rem; margin-bottom: 0.75rem; font-size: 0.9rem; font-weight: 700; text-transform: uppercase;">3. Family & Financial Status</h4>
                        <table style="width: 100%; border-collapse: collapse; font-size: 0.85rem; color: var(--text-main);">
                            <tr style="border-bottom: 1px solid rgba(255,255,255,0.02);"><td style="padding: 0.5rem 0; font-weight: 600; width: 140px; color: var(--text-muted);">Children Count:</td><td>${numChildren}</td></tr>
                            <tr style="border-bottom: 1px solid rgba(255,255,255,0.02);"><td style="padding: 0.5rem 0; font-weight: 600; color: var(--text-muted);">Health Status:</td><td>${healthStatus}</td></tr>
                            <tr style="border-bottom: 1px solid rgba(255,255,255,0.02);"><td style="padding: 0.5rem 0; font-weight: 600; color: var(--text-muted);">Monthly Income:</td><td>${monthlyIncome}</td></tr>
                        </table>

                        <h4 style="color: var(--accent-cyan); border-bottom: 1px solid var(--panel-border); padding-bottom: 0.5rem; margin-top: 1.5rem; margin-bottom: 0.75rem; font-size: 0.9rem; font-weight: 700; text-transform: uppercase;">4. Proposed Project Details</h4>
                        <table style="width: 100%; border-collapse: collapse; font-size: 0.85rem; color: var(--text-main);">
                            <tr style="border-bottom: 1px solid rgba(255,255,255,0.02);"><td style="padding: 0.5rem 0; font-weight: 600; width: 140px; color: var(--text-muted);">Desired Model:</td><td style="text-transform: capitalize; font-weight: 600; color: var(--text-main);">${desiredModel}</td></tr>
    ...
    <script>
        ...
        function openEditContractorModal(index, contractor) {
            if (!contractor && window.projectContractorsList && window.projectContractorsList[index]) {
                contractor = window.projectContractorsList[index];
            }
