...
@section('title', 'Family Aid Project List')
...
<div class="no-auto-align" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem; margin-bottom: 1.5rem; background: #ffffff; padding: 1.25rem 1.5rem; border-radius: 16px; border: 1px solid #e2e8f0; box-shadow: 0 4px 20px -2px rgba(15, 23, 42, 0.03);">
    <div>
        <div style="display: flex; align-items: center; gap: 0.65rem; flex-wrap: wrap;">
            <h1 style="font-size: 1.3rem; font-weight: 800; color: #0f172a; margin: 0; letter-spacing: -0.02em;">Family Aid Projects</h1>
    ...
    <div style="overflow-x: auto;">
        <table class="table-custom" id="projectsTable">
            ...
            <tbody>
                @forelse($projects as $index => $project)
                    ...
                    <tr class="project-row" 
                        ...
                        <td style="font-weight: 600; color: var(--accent-cyan);">
                            <a href="{{ route('projects.show', $project->id) }}?type={{ urlencode($project->type_of_project ?? 'Family Aid') }}" style="color: var(--accent-cyan); font-weight: 700; text-decoration: underline;" title="View Project Details">
                                {{ $project->project_id }}
                            </a>
                        </td>
                        <td>{{ $project->agency_project_no ?? ($app?->agency_number ?? ($appMeta['agency_number'] ?? 'N/A')) }}</td>
                        <td>{{ $rowAgencyName }}</td>
                        <td>{{ $app?->applicant_name ?? ($appMeta['applicant_name'] ?? 'N/A') }}</td>
                        <td>{{ $app?->mother_name ?? ($appMeta['mother_name'] ?? ($app?->father_name ?? ($appMeta['father_name'] ?? 'N/A'))) }}</td>
                        ...
                        <td style="text-align: center; white-space: nowrap;">
                            ...
                            <a href="{{ route('projects.show', $project->id) }}?type={{ urlencode($project->type_of_project ?? 'Family Aid') }}" class="btn-action-icon btn-dots" title="Details">
                                <i class="bx bx-dots-horizontal-rounded"></i>
                            </a>

                            <!-- PDF Report Button -->
                            <a href="{{ route('projects.pdf', [$project->id, 'category' => 'family-aid']) }}" onclick="downloadDirectPdf(event, this.href)" class="btn-action-icon btn-pdf" title="Download PDF Report">
                ...
                @empty
                    <tr>
                        <td colspan="9" style="text-align: center; padding: 2rem; color: var(--text-muted);">No family aid projects registered yet.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
<script>
    ...
    function updateExportUrl() {
        ...
        const baseUrl = "{{ route('projects.export', 'family-aid') }}";
    ...
    window.openAddProgrammeModal = function openAddProgrammeModal(param) {
        ...
        if (modal) {
            ...
            if (projectId) {
                ...
                if (form) {
                    form.action = `/admin/projects/family-aid/${projectId}/add-programme`;
                }
            }
...
<div class="modal-overlay" id="addProgrammeModal" style="display: none;">
    <div class="modal-content-custom" style="max-width: 600px; max-height: 90vh; overflow-y: auto; background-color: var(--panel-bg); border: 1px solid var(--panel-border); padding: 2rem; border-radius: 12px;">
        <div class="modal-header-custom" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
            ...
            <button type="button" class="modal-close-btn" onclick="closeAddProgrammeModal()" title="Close">&times;</button>
        </div>
        ...
        <form id="addProgrammeForm" method="POST" action="" onsubmit="handleAddProgrammeSubmit(event); return false;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1.5rem;">
                <div style="grid-column: span 2;">
                    ...
                    <select name="programme_name" id="fa_add_prog_name_select" required class="form-control-dark" style="width: 100%; padding: 0.6rem 0.8rem; border-radius: 6px; border: 1px solid var(--panel-border); background-color: var(--bg-color); color: #ffffff;">
