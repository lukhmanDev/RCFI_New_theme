...
@section('title', 'General Project List')
...
<div class="no-auto-align" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem; margin-bottom: 1.5rem; background: #ffffff; padding: 1.25rem 1.5rem; border-radius: 16px; border: 1px solid #e2e8f0; box-shadow: 0 4px 20px -2px rgba(15, 23, 42, 0.03);">
    <div>
        <div style="display: flex; align-items: center; gap: 0.65rem; flex-wrap: wrap;">
            <h1 style="font-size: 1.3rem; font-weight: 800; color: #0f172a; margin: 0; letter-spacing: -0.02em;">General Projects</h1>
        ...
        <p style="font-size: 0.82rem; color: #64748b; margin: 0.2rem 0 0 0;">Manage, filter, monitor progress, and export general projects.</p>
    </div>
...
@php
    ...
    foreach($projects as $p) {
        ...
        $d = $p->district ?? ($p->application ? ($p->application->district ?? ($p->application->meta['district'] ?? ($project->application->meta['locality_district'] ?? null))) : null);
    ...
    <div style="overflow-x: auto;">
        <table class="table-custom" id="projectsTable">
            ...
            <tbody>
                @forelse($projects as $index => $project)
                    ...
                    <tr class="project-row" 
                        ...
                        <td style="font-weight: 600; color: var(--accent-cyan);">
                            <a href="{{ route('projects.show', $project->id) }}?type={{ urlencode($project->type_of_project ?? 'General') }}" style="color: var(--accent-cyan); font-weight: 700; text-decoration: underline;" title="View Project Details">
                                {{ $project->project_id }}
                            </a>
                        </td>
                        ...
                        <td style="text-align: center; white-space: nowrap;">
                            ...
                            <!-- PDF Report Button -->
                            <a href="{{ route('projects.pdf', [$project->id, 'category' => 'general']) }}" onclick="downloadDirectPdf(event, this.href)" class="btn-action-icon btn-pdf" title="Download PDF Report">
                ...
                @empty
                    <tr>
                        <td colspan="12" style="text-align: center; padding: 2rem; color: var(--text-muted);">No general projects registered yet.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
<div class="modal-overlay" id="addProjectModal">
    <div class="modal-content-custom">
        ...
        <form action="{{ route('projects.store') }}" method="POST">
            @csrf
            <input type="hidden" name="redirect_category" value="general">
            ...
            <div class="modal-body-custom">
                ...
                <div class="form-group-custom">
                    ...
                    <input type="text" value="General" disabled style="background-color: var(--bg-color); border: 1px solid var(--panel-border); color: var(--text-muted);">
                    <input type="hidden" name="type_of_project" value="General">
                </div>
...
<script>
    ...
    function updateExportUrl() {
        ...
        const baseUrl = "{{ route('projects.export', 'general') }}";
