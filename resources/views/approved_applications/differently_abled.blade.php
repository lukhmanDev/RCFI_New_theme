...
@section('title', 'Approved Differently Abled Applications')
...
@section('content')
                ...
                <tbody>
                    @forelse($applications as $appItem)
                        ...
                        <tr class="app-row" id="app-row-{{ $appItem->id }}" data-search="{{ $searchStr }}" data-cluster="{{ $appItem->cluster_id ?? '' }}" data-sponsor="{{ strtolower($appItem->sponsor_status ?? 'not sponsored') }}" data-place="{{ $appItem->place ?? '' }}" onclick="openDetailsModal({{ $appItem->id }})">
                            ...
                            <td>{{ $meta['gender'] ?? '-' }}</td>
                            <td>{{ $meta['age'] ?? '-' }}</td>
                            <td title="{{ $meta['disability_type'] ?? '-' }}">{{ \Illuminate\Support\Str::limit($meta['disability_type'] ?? '-', 15, '...') }}</td>
    ...
    <script>
        ...
        function onSponsorStatusUpdated(appId, newStatus, categorySlug = 'differently-abled', sponsoredDate = null) {
            if (!appId) return;
